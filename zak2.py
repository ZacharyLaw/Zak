...
@client.command(aliases=(['weapon','weapons']))
...
async def ship(ctx):
	if ctx.message.content.endswith('ships') or ctx.message.content.endswith('ship'):await ctx.send(file=discord.File('ship.png'))
@client.command()
